...
constexpr char kNormalizedFilteredLandmarksTagv2[] = "NORM_FILTERED_LANDMARKS_V2";
...
//   }
//
class LandmarksSmoothingCalculatorV2 : public CalculatorBase {
 public:
...
REGISTER_CALCULATOR(LandmarksSmoothingCalculatorV2);

mediapipe::Status LandmarksSmoothingCalculatorV2::GetContract(
  ...
  cc->Outputs()
      .Tag(kNormalizedFilteredLandmarksTagv2)
      .Set<NormalizedLandmarkList>();

  return mediapipe::OkStatus();
}

mediapipe::Status LandmarksSmoothingCalculatorV2::Open(CalculatorContext* cc) {
...
mediapipe::Status LandmarksSmoothingCalculatorV2::Process(CalculatorContext* cc) {
  ...
  if (cc->Inputs().Tag(kNormalizedLandmarksTag).IsEmpty()) {
    ...
    auto emptyList = absl::make_unique<NormalizedLandmarkList>();
    cc->Outputs()
        .Tag(kNormalizedFilteredLandmarksTagv2)
        .Add(emptyList.release(), cc->InputTimestamp());
    return mediapipe::OkStatus();
  }
  ...
  // copy landmarks
  auto out_landmarks2 = absl::make_unique<NormalizedLandmarkList>();
  for (int i = 0; i < out_landmarks->landmark_size(); ++i) {
    const NormalizedLandmark& landmark = out_landmarks->landmark(i);
    NormalizedLandmark* out_landmark2 = out_landmarks2->add_landmark();
    *out_landmark2 = landmark;
  }
  cc->Outputs()
      .Tag(kNormalizedFilteredLandmarksTag)
  ...
  cc->Outputs()
      .Tag(kNormalizedFilteredLandmarksTagv2)
      .Add(out_landmarks2.release(), cc->InputTimestamp());
